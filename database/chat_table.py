...
async def get_chat(chat_id: int) -> ChatModel | None:
    async with connect(BASE_PATH) as db:
        cursor = await db.execute(
            "SELECT chat_id, chat_title, chat_captcha_enabled, chat_captcha_timeout "
...
async def add_chat(
    ...
    async with connect(BASE_PATH) as db:
        await db.execute(
            "INSERT INTO chat_table (chat_id, chat_title, chat_captcha_enabled, chat_captcha_timeout) "
            "VALUES (?, ?, ?, ?)",
            (chat_id, chat_title, chat_captcha_enabled, chat_captcha_timeout)
        )
        await db.commit()
...
# ~~~~ DATA UPDATING ~~~~
ALLOWED_CHAT_FIELDS = {"chat_title", "chat_captcha_enabled", "chat_captcha_timeout"}
