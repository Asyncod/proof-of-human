...
async def get_captcha(captcha_user_id: int, captcha_chat_id: int) -> CaptchaModel | None:
    async with connect(BASE_PATH) as db:
        cursor = await db.execute(
            "SELECT captcha_user_id, captcha_chat_id, captcha_expires_at, captcha_payload, captcha_message_id, captcha_correct_emoji, captcha_user_message_id "
...
async def add_captcha(
    ...
    async with connect(BASE_PATH) as db:
        await db.execute(
            "INSERT INTO captcha_table (captcha_user_id, captcha_chat_id, captcha_expires_at, captcha_payload, captcha_message_id, captcha_correct_emoji, captcha_user_message_id) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (captcha_user_id, captcha_chat_id, captcha_expires_at, captcha_payload, captcha_message_id, captcha_correct_emoji, captcha_user_message_id)
        )
        await db.commit()
